              ...
              <tbody>
                @forelse($recentMeetings as $meeting)
                  <tr>
                    ...
                    <td>
                      <span class="badge bg-{{ $meeting->status === 'completed' ? 'success' : ($meeting->status === 'scheduled' ? 'warning' : 'secondary') }}">
                        {{ $meeting->status === 'completed' ? 'Đã hoàn thành' : ($meeting->status === 'scheduled' ? 'Đã lên lịch' : 'Chưa xác định') }}
                      </span>
                    </td>
                  </tr>
                @empty
                  <tr><td colspan="5" class="text-center">Chưa có cuộc họp nào</td></tr>
                @endforelse
              </tbody>
            </table>
          </div>
